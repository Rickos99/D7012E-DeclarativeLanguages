...
value (Num n) _ = n
value (Var v) mem = case Dictionary.lookup v mem of
        Just a -> a
        Nothing -> error ("Variable not in scope: " ++ v)
value (Add e1 e2) mem = value e1 mem + value e2 mem
value (Sub e1 e2) mem = value e1 mem - value e2 mem
value (Mul e1 e2) mem = value e1 mem * value e2 mem
value (Div e1 e2) mem = value e1 mem `div` case value e2 mem of
        0 -> error "Division by zero encountered"
        v -> v
